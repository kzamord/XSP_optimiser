...
def get_zone_label(sbx, sby):
    ...
    if   sby < 16.8:  lane = "Left wide"
    elif sby < 29.6:  lane = "Left half"
    elif sby < 36.3:  lane = "Left central"
    elif sby < 43.7:  lane = "Central"
    elif sby < 50.4:  lane = "Right central"
    elif sby < 63.2:  lane = "Right half"
...
def generate_grid(mode, params, resolution=50):
    if mode == "Corner":
        # Scan shot positions inside box — matching training data distribution
...
ax_pitch.axhline(FINAL_THIRD, color="white", linewidth=1.5,
                  linestyle="--", alpha=0.7, zorder=6)
ax_pitch.axhline(BOX_ZONE, color="white", linewidth=0.9,
                  linestyle="--", alpha=0.5, zorder=6)
for xl in [16.8, 29.6, 50.4, 63.2]:
    ax_pitch.plot([xl,xl],[FINAL_THIRD,120], color="white",
                   linewidth=0.7, linestyle="--", alpha=0.4, zorder=6)
# Near post / far post — orange dotted, only in box zone
for xl in [36.3, 43.7]:
    ax_pitch.plot([xl,xl],[BOX_ZONE,120], color="#F2A623",
                   linewidth=0.8, linestyle=":", alpha=0.7, zorder=6)

lane_centers = [8.4, 23.2, 33.0, 40.0, 47.0, 56.8, 71.6]
zone_names   = ["Left wide","Left half","Left central","Central","Right central","Right half","Right wide"]
deep_mid     = (FINAL_THIRD+BOX_ZONE)/2
box_mid      = (BOX_ZONE+120)/2
for xc, name in zip(lane_centers, zone_names):
    ax_pitch.text(xc, deep_mid, name, color="white", fontsize=7,
                   ha="center", va="center", alpha=0.6, zorder=7)
    ax_pitch.text(xc, box_mid,  name, color="white", fontsize=7,
                   ha="center", va="center", alpha=0.6, zorder=7)
ax_pitch.text(-5, deep_mid, "Deep zone", color="white", fontsize=8,
               ha="center", va="center", alpha=0.8, rotation=90)
ax_pitch.text(-5, box_mid,  "Box zone",  color="white", fontsize=8,
               ha="center", va="center", alpha=0.8, rotation=90)

bi = np.unravel_index(grid.argmax(), grid.shape)
best_dot, = ax_pitch.plot(
    y_vals[bi[0]], x_vals[bi[1]], "*",
    color="#FFD700", markersize=22,
    markeredgecolor="white", markeredgewidth=1.5, zorder=10
)
info_text = ax_pitch.text(
    40, 82, "Best xSP: —",
    color="white", fontsize=9, ha="center",
    bbox=dict(facecolor="black", alpha=0.6, edgecolor="#FFD700",
              linewidth=1, boxstyle="round,pad=0.4"), zorder=11
)
warn_text = ax_pitch.text(
    40, 78, "",
    color="#FF5252", fontsize=8, ha="center", va="top", zorder=15,
    bbox=dict(facecolor="black", alpha=0.7, edgecolor="#FF5252",
              linewidth=1, boxstyle="round,pad=0.3")
)
click_dot, = ax_pitch.plot([], [], "o", color="#4FC3F7", markersize=12,
                            markeredgecolor="white", markeredgewidth=2, zorder=12)
click_text = ax_pitch.text(
    0, 0, "", color="white", fontsize=8, ha="center", va="bottom", zorder=13,
    bbox=dict(facecolor="black", alpha=0.6, edgecolor="#4FC3F7",
              linewidth=1, boxstyle="round,pad=0.3")
)
...
ax_pitch.set_title("xSP Optimiser - Direct Free Kick",
                    color="#F2A623", fontsize=12, fontweight="bold", pad=12)
...
fk_sliders = {}
fk_specs   = [
    ("defenders", "Defenders in box", 0,  10,  1,   5),
    ("attackers", "Attackers",        0,   9,  1,   7),
    ("wall",      "Wall defenders",   0,  10,  1,   5),
    ("end_z",     "Shot height (m)",  0.1, 2.8, 0.1, 1.2),
]
fk_title = fig.text(0.53, 0.875, "Free Kick parameters",
                     color="#F2A623", fontsize=10, fontweight="bold")
...
for i, (key, lbl, vmin, vmax, step, vdef) in enumerate(fk_specs):
    ...
    sl = Slider(ax_sl, lbl, vmin, vmax, valinit=vdef, valstep=step,
                color="#F2A623", track_color="#444")
...
fk_opp_title = fig.text(0.53, 0.355, "Opposition team",
                         color="#aaaaaa", fontsize=8, style="italic")
...
fk_selected_text = ax_fk_drop_display.text(
    0.5, 0.5, "Average", color="white", fontsize=10, ha="center", va="center",
    bbox=dict(facecolor="#2a2a2a", edgecolor="#F2A623",
              linewidth=1.5, boxstyle="round,pad=0.5")
)
...
# ── Shared list (created once, used for both FK and Corner) ──────────────
shared_list_axes = []
shared_list_btns = []
FK_LIST_SIZE     = 5
fk_list_start    = [0]
...
for i in range(FK_LIST_SIZE):
    ax_li = fig.add_axes([0.55, 0.190 - i*0.036, 0.40, 0.030])
    ...
    shared_list_axes.append(ax_li)
    shared_list_btns.append(btn_li)

fk_list_axes = shared_list_axes
fk_list_btns = shared_list_btns
c_list_axes  = shared_list_axes
c_list_btns  = shared_list_btns

fk_axes += [ax_fk_drop_display, ax_fk_prev, ax_fk_next]
...
corner_axes    = []
corner_sliders = {}
corner_title   = fig.text(0.53, 0.875, "Corner parameters",
...
sl_delivery = Slider(ax_del, "Delivery type", 0, 2,
                     valinit=0, valstep=1, color="#4FC3F7", track_color="#444")
...
lbl_del_title = fig.text(0.53, 0.500, "Delivery type",
                          color="#aaaaaa", fontsize=8, style="italic")
...
sl_side = Slider(ax_side_sl, "Corner side", 0, 1,
                 valinit=0, valstep=1, color="#4FC3F7", track_color="#444")
...
c_opp_title = fig.text(0.53, 0.295, "Opposition team",
                        color="#aaaaaa", fontsize=8, style="italic")
...
c_selected_text = ax_c_drop_display.text(
    0.5, 0.5, "Average", color="white", fontsize=10, ha="center", va="center",
    bbox=dict(facecolor="#1a3a3a", edgecolor="#4FC3F7",
              linewidth=1.5, boxstyle="round,pad=0.5")
)
...
corner_axes += [ax_c_drop_display, ax_c_prev, ax_c_next]
all_corner_extra = [
    corner_title, lbl_del_title, lbl_ins, lbl_str, lbl_out,
    lbl_side_title, lbl_left_c, lbl_right_c, c_opp_title
]
...
def apply_fk_opp(name):
    ...
    if name == "Average":
        ...
        fk_sliders["defenders"].set_val(5)
        fk_sliders["attackers"].set_val(7)
    elif name in fk_teams_dict:
        p    = fk_teams_dict[name]
        wall = p["wall"]
        defs = min(p["defenders"], 10 - wall)
        fk_sliders["wall"].set_val(wall)
        fk_sliders["defenders"].set_val(defs)
...
for i, btn_li in enumerate(shared_list_btns):
    btn_li.on_clicked(make_shared_click(i))
...
def fk_next(event):
    fk_list_start[0] = min(len(fk_teams_sorted) - FK_LIST_SIZE,
                           fk_list_start[0] + FK_LIST_SIZE)
    update_fk_list()
...
def c_next(event):
    c_list_start[0] = min(len(corner_teams_sorted) - FK_LIST_SIZE,
                          c_list_start[0] + FK_LIST_SIZE)
    update_c_list()
...
# ════════════════════════════════════════
# HELPER: clear player dots
# ════════════════════════════════════════
def clear_player_dots():
    for dot in player_dots:
        try: dot.remove()
        except: pass
    player_dots.clear()

def draw_player_dots(sbx, sby, mode, params):
    clear_player_dots()
    if mode == "FK":
        wall  = params["wall"]
        ...
        for off in offsets:
            ...
            dot, = ax_pitch.plot(wy, wx, 'o', color='#E53935', markersize=10,
        ...
        for i in range(defs):
            dot, = ax_pitch.plot(def_y[i % len(def_y)], def_x[i % len(def_x)],
                's', color='#B71C1C', markersize=8,
        ...
        for i in range(atts):
            dot, = ax_pitch.plot(att_y[i % len(att_y)], att_x[i % len(att_x)],
                '^', color='#1565C0', markersize=9,
                markeredgecolor='white', markeredgewidth=0.8, zorder=15)
            player_dots.append(dot)
        for lx, ly, sym, col, lbl in [
            (10, FINAL_THIRD-5, '●', '#E53935', 'Wall ('+str(wall)+')'),
            (30, FINAL_THIRD-5, '■', '#B71C1C', 'Def (' +str(defs)+')'),
            (55, FINAL_THIRD-5, '▲', '#1565C0', 'Att (' +str(atts)+')'),
        ]:
            dot = ax_pitch.text(lx, ly, sym+' '+lbl, color=col,
                fontsize=7.5, fontweight='bold', ha='center', va='center', zorder=14)
            player_dots.append(dot)
    else:
        ...
        for i in range(atts_box):
            ...
            player_dots.append(dot)
        for lx, ly, sym, col, lbl in [
            (20, FINAL_THIRD-5, '■', '#E53935', 'Def ('+str(defs_box)+')'),
            (50, FINAL_THIRD-5, '▲', '#1565C0', 'Att ('+str(atts_box)+')'),
        ]:
            dot = ax_pitch.text(lx, ly, sym+' '+lbl, color=col,
                fontsize=7.5, fontweight='bold', ha='center', va='center', zorder=14)
            player_dots.append(dot)
...
def get_params():
    ...
    if mode == "FK":
        ...
        total     = wall + defenders
        if total > 10:
            warn_text.set_text(
                "⚠️ Wall ("+str(wall)+") + Def ("+str(defenders)+") = "+str(total)+" > 10 — Def capped to "+str(10-wall)
            )
            defenders = 10 - wall
        else:
            warn_text.set_text("")
        return mode, dict(
            ...
            wall      = wall,
            end_z     = round(float(fk_sliders["end_z"].val), 1),
        )
    else:
        warn_text.set_text("")
        return mode, dict(
            ...
            end_z         = round(float(corner_sliders["end_z"].val), 1),
...
def refresh(event=None):
    ...
    info_text.set_text(
        "Best xSP: " + str(round(new_grid.max()*100, 1)) + "%" +
        " | Dist: " + str(round(bdist, 1)) + "m" +
        " | " + bzone
    )
    ...
    if mode == "FK":
        ...
        subtitle = " | vs "+opp+" | Wall:"+str(params["wall"])+" | H:"+str(round(params["end_z"],1))+"m"
        color = "#F2A623"
    ...
    ax_pitch.set_title(
        "xSP Optimiser - "+label+subtitle,
        color=color, fontsize=10, fontweight="bold", pad=12
    )
    draw_player_dots(bx, by, mode, params)
    fig.canvas.draw_idle()
...
def on_click(event):
    ...
    draw_player_dots(sbx, sby, mode, params)
    ...
    click_text.set_text(
        "xSP: "+str(round(xsp*100,1))+"% ["+qual+"] | "+str(round(dist,1))+"m | "+zone
    )
    fig.canvas.draw_idle()
...
fig.text(0.26, 0.97, "xSP Optimiser - Free Kick & Corner",
         color="white", fontsize=14, fontweight="bold", ha="center")
fig.text(0.26, 0.94,
         "Select mode | Pick opposition | Adjust sliders | Click pitch",
