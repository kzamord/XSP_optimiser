...
with st.spinner("⚽ Loading models and profiles..."):
...
def build_figure(mode, params, opp_name):
    x_vals, y_vals, grid = generate_grid(mode, params)
    ...
    for xc, name in zip(lane_centers, zone_names):
        ax.text(xc, deep_mid, name, color="white", fontsize=6,
                ha="center", va="center", alpha=0.6, zorder=7)
        ax.text(xc, box_mid, name, color="white", fontsize=6,
                ha="center", va="center", alpha=0.6, zorder=7)
    ...
    ax.text(40, 82,
            f"★ Best xSP: {grid.max():.4f}  |  {bdist:.1f}m  |  {bzone}",
    ...
    return fig, grid.max(), bdist, bzone
...
with st.sidebar:
    ...
    mode = st.radio("Mode", ["Free Kick", "Corner"],
                    horizontal=True)
    st.markdown("---")
    ...
    if mode == "Free Kick":
        ...
        # Auto-fill sliders from profile
        if opp == "Average":
            def_wall, def_def, def_att = 5, 6, 7
        elif opp in fk_teams_dict:
            p = fk_teams_dict[opp]
            def_wall, def_def, def_att = p["wall"], p["defenders"], p["attackers"]
        else:
            def_wall, def_def, def_att = 5, 6, 7

        wall      = st.slider("Wall defenders",   0, 10, def_wall)
        defenders = st.slider("Defenders in box", 1, min(11, 11 - wall), min(def_def, 11 - wall))
        attackers = st.slider("Attackers",        0,  9, def_att)
        end_z     = st.slider("Shot height (m)",  0.1, 2.8, 1.2, step=0.1)

        params = dict(wall=wall, defenders=defenders, attackers=attackers, end_z=end_z)
        ...
        end_z         = st.slider("Shot height (m)",  0.1, 2.8, 1.2, step=0.1)
        delivery      = st.select_slider("Delivery type", ["Inswinging","Straight","Outswinging"])
        ...
        params = dict(defenders_box=defenders_box, attackers_box=attackers_box,
...
# ── Main area ─────────────────────────────────────────────────────────────
st.markdown(f"<h1 style='text-align:center'>⚽ xSP Optimiser</h1>", unsafe_allow_html=True)
st.markdown(f"<p style='text-align:center; color:#aaaaaa'>Select mode · Pick opposition · Adjust sliders · Click Refresh</p>",
            unsafe_allow_html=True)

mode_key = "FK" if mode == "Free Kick" else "Corner"

if run or "grid_fig" not in st.session_state or st.session_state.get("last_mode") != mode_key:
    with st.spinner("Computing xSP heatmap..."):
        fig, best_xsp, best_dist, best_zone = build_figure(mode_key, params, opp)
    st.session_state["grid_fig"]  = fig
...
color = "#F2A623" if mode_key == "FK" else "#4FC3F7"
col1.metric("Best xSP", f"{st.session_state['best_xsp']:.4f}")
col2.metric("Distance", f"{st.session_state['best_dist']:.1f} m")
col3.metric("Best zone", st.session_state["best_zone"])

st.pyplot(st.session_state["grid_fig"], use_container_width=True)
