...
def build_figure(mode, opp_name, wall=0, defenders=6, attackers=7, end_z=1.2,
    ...
    bzone = ("Box zone" if bx >= BOX_ZONE else "Deep zone") + " - " + (
        "Left wide"  if by < 16.8 else
        "Left half"  if by < 29.6 else
        "Central"    if by < 50.4 else
        "Right half" if by < 63.2 else "Right wide"
    )
